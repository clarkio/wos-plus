...
| # | Gap | Pinned by |
| --- | --- | --- |
| ~~X1~~ | ~~All three Supabase-backed routes advertise `OPTIONS` in `Access-Control-Allow-Methods` while exporting **no `OPTIONS` handler**, so a real CORS preflight falls through to a 404. Reachable in practice for `PUT /api/boards/[id]`, which does preflight from a browser.~~ **Fixed** — all three routes (`/api/boards`, `/api/boards/[id]`, `/api/channel-stats/[channel]`) now export an `OPTIONS` handler answering a 204 with the same static CORS headers the route already sends, matching the pattern `/api/words` used via `src/lib/cors.ts`. Per [#172](https://github.com/clarkio/wos-plus/issues/172). | "exports the OPTIONS handler its Access-Control-Allow-Methods promises (fixed #172)" in `boards.acceptance.test.ts` (×2) and `channel-stats.acceptance.test.ts` |
